...
- **Use GitHub for adoption signals.** \`discoverCompaniesFromGitHub\` is the fastest path from "who uses a competitor's SDK or a category library" to a ranked list of companies — pass anchor repos (e.g. \`["privy-io/privy-js", "thirdweb-dev/js"]\`) and it clusters their stargazers' employers into companies. Use \`searchGitHubRepos\` first if you don't know a repo's exact path. \`fetchGitHubStargazers\` + \`enrichGitHubProfiles\` remain for people-level digging.
